...
``` r
# Extrair o modelo ranger treinado
fco2_rf_ranger_model <- fco2_rf_last_fit$.workflow[[1]] |>
  extract_fit_engine()

# Preparar os dados de teste processados pela recipe
fco2_test_processed <- fco2_rf_last_fit$.workflow[[1]] |>
  extract_recipe() |>
  bake(new_data = fco2_test)

# Remover a variável resposta se estiver presente
fco2_test_features <- fco2_test_processed |>
  select(-fco2)  # ajuste o nome se necessário

# Calcular SHAP via treeshap
unified_model <- ranger.unify(fco2_rf_ranger_model, fco2_test_features)
shap_values   <- treeshap(unified_model, fco2_test_features, verbose = FALSE)

# Criar objeto shapviz para visualizações
...
``` r
# 1. Beeswarm plot (substituto moderno do summary plot do Python)
sv_importance(sv, kind = "beeswarm")

# 2. Importância média |SHAP| (bar plot)
sv_importance(sv, kind = "bar", max_display = 15)

# 3. Dependence plot — efeito de uma variável específica
sv_dependence(sv, v = "inso")  # substitua pelo nome real

# 4. SHAP para uma única observação (waterfall plot)
sv_waterfall(sv, row_id = 1)

# 5. Force plot para uma observação
